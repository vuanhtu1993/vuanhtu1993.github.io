---
title: "Tiến Hóa Công Nghệ AI (2017→Tương Lai)"
date: "2026-04-21"
authors: [anhhtus]
...
```mermaid
flowchart TD
    A["🧠 Phase 1<br>LLM Ra Đời - Base model<br>(2017-2021)"]
    B["💬 Phase 2<br>ChatGPT + Prompting<br>(2022-2023)"]
    C["🤖 Phase 3<br>AI Agent<br>(2023-2025)"]
    D["🎼 Phase 4<br>Agent Orchestra<br>(2025-2027)"]
    E["🔮 Phase 5<br>Tương Lai<br>(2027+)"]
...
```mermaid
flowchart LR
    subgraph "NLP hoạt động"
        IN["📝 Input Text<br>\"Con mèo ngồi\""]
        ATT["🔍 Self-Attention<br>Từ nào liên quan<br>đến từ nào?"]
        OUT["📤 Output<br>\"...trên thảm\""]
    end
    IN --> ATT --> OUT
```
...
### Pain Point — Vấn đề phát sinh

> **GPT-3 thô và nguy hiểm.** Model có thể viết bất kỳ thứ gì — kể cả nội dung độc hại, sai lệch. Đồng thời, interface vẫn là API kỹ thuật, người thường không thể tiếp cận.
...
```mermaid
flowchart TD
    PROB["Doanh nghiệp muốn<br>dùng LLM cho thực tế"]

    W1["🧠 Bức tường 1: Hallucination<br>Model tự tin bịa đặt thông tin<br>không tồn tại"]
    W2["📅 Bức tường 2: Knowledge Cutoff<br>Model không biết gì sau<br>ngày training kết thúc"]
    W3["🏢 Bức tường 3: No Domain Knowledge<br>Model không biết SOP, sản phẩm,<br>hay quy trình nội bộ của công ty"]
...
> **Vấn đề cốt lõi:** LLM là một "bộ não khổng lồ nhưng bị cô lập". Nó không thể tự tìm kiếm thêm thông tin. Nó không thể gọi API. Nó không có tác vụ — chỉ có phản hồi. Và đây là lúc các **Enabling Technologies** ra đời.

---

## Phase 3: Enabling Technologies - Mở rộng khả năng của AI
...
flowchart LR
    subgraph TOOLS["🔧 Enabling Technologies Layer"]
        FC["⚡ Function Calling<br>(2023)<br>LLM gọi được API ngoài"]
        RAG["🔍 RAG<br>(2023)<br>Grounding với<br>data thực tế"]
        FT["🎯 Fine-tuning<br>(2022-nay)<br>Domain expertise<br>vào model"]
        MCP["🔌 MCP<br>(2024)<br>Chuẩn hóa<br>tool access"]
    end

    FC -->|"LLM có thể hành động"| AGENT["🧠 LLM"]
    RAG -->|"LLM có memory thực"| AGENT
    FT -->|"LLM hiểu domain"| AGENT
    MCP -->|"Kết nối 50+ tools<br>không cần custom code"| AGENT
...
## Phase 4: AI Agent - Từ Hỏi Đáp Đến Hành Động
### Bài toán cần giải quyết
...
```mermaid
flowchart LR
    P["👁 Perceive<br>Nhận task<br>và context"] --> T["🧠 Think<br>Phân tích,<br>lập kế hoạch"]
    T --> A["⚡ Act<br>Gọi tool,<br>thực thi bước"]
    A --> O["🔍 Observe<br>Kiểm tra<br>kết quả"]
    O -->|"Chưa xong"| T
    O -->|"Hoàn thành"| R["✅ Report<br>Báo cáo<br>cho người dùng"]
```
...
## 🎼 Phase 5: Agent Orchestra — "Dàn Nhạc Giao Hưởng"
...
```mermaid
flowchart TB
    USER["👤 Người dùng<br>'Phân tích market và ra chiến lược'"]
    ORCH["🎼 Orchestrator Agent<br>Nhận task, phân vai,<br>quản lý flow"]

    R_AGT["🔍 Research Agent<br>Chuyên thu thập thông tin"]
    A_AGT["📊 Analysis Agent<br>Chuyên phân tích số liệu"]
    W_AGT["✍️ Writing Agent<br>Chuyên soạn thảo văn bản"]
    QA_AGT["🛡️ QA Agent<br>Chuyên kiểm tra chất lượng"]
...
## Phase 6: Future — 3 Kịch Bản Đang Tranh Luận
...
flowchart BT
    LLM["🧠 LLM<br>(Engine)"]
    AGENT["🤖 Agent<br>= LLM + Tools + Loop"]
    MULTI["🎼 Multi-Agent<br>= Agents + Orchestration"]
    FUTURE["🔮 Future<br>= Multi-Agent + Physical + Governance"]
