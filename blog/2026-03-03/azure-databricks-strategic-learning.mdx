...
df_last_week = spark.read.format("delta") \
    .option("versionAsOf", 30) \  # Version 30 = 7 ngày trước
    .load("/data/inventory")
...
flowchart LR
    subgraph "Giai đoạn 1 - Nền tảng\n(Tháng 1-2)"
        ...
        A2["⚡ Apache Spark cơ bản\n(PySpark, SQL)"]
        A3["🗂️ Delta Lake 101\n(CRUD, Time Travel)"]
    end

    subgraph "Giai đoạn 2 - Chuyên sâu\n(Tháng 3-5)"
        B1["🔧 ETL Pipelines\n(Lakeflow/DLT)"]
        B2["📊 Databricks SQL\n(Dashboard, BI)"]
        B3["🤖 MLflow cơ bản\n(Tracking, Registry)"]
    end

    subgraph "Giai đoạn 3 - Chuyên gia\n(Tháng 6+)"
        C1["🛡️ Unity Catalog\n(Governance)"]
        C2["🚀 MLOps Production\n(Deploy, Monitor)"]
        C3["🎗️ Certification\n(DE/DA/ML Associate)"]
    end
...
```mermaid
gantt
    title Lộ trình 12 tuần - Data Engineer Associate
    dateFormat  YYYY-MM-DD
    section Nền tảng
    Databricks Architecture & Setup     :a1, 2026-03-03, 1w
    ...
    section Governance & Ôn thi
    Unity Catalog                       :d1, after c2, 1w
    Practice Exams + Review             :d2, after d1, 2w
```
...
mindmap
  root(("Azure Databricks"))
    Concept["Khái niệm cốt lõi"]
      ...
      prob["Giải quyết: Data silo,\nETL phức tạp, AI-ready"]
    ...
    Components["Core Components"]
      ...
      unity["Unity Catalog\n(Governance)"]
    UseCases["Use Cases"]
      when["Khi cần: ETL phức tạp,\nML + Analytics cùng lúc"]
      notwhen["Không dùng: Chỉ cần SQL BI đơn giản\n→ Dùng Snowflake thay thế"]
      example["Banking: Fraud Detection\nRetail: Inventory AI"]
    BestPractices["Best Practices"]
      conv["Học Spark trước, Databricks sau"]
      sec["Unity Catalog cho mọi workspace"]
      perf["Auto-terminate cluster để tiết kiệm"]
```
