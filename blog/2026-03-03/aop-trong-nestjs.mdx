...
Để hình dung rõ nhất về vòng đời của một Request trong NestJS và cách các "Trạm Kiểm Soát" phân bổ, hãy xem qua sơ đồ trực quan dưới đây:
...
> **Phân tích sơ đồ:**
> 1. **Middleware (1)**: Đón đầu Request đầu tiên trước khi đi vào hệ thống.
> 2. **Guard (2)**: Đứng ngay sau đó để quyết định (Authorization) xem Request có quyền đi thẳng tiếp hay không.
> 3. **Interceptor (4 - Trước khi Handler xử lý)**: Can thiệp, biến đổi request trước khi chạm đến logic chính.
> 4. **Pipe (3) & Handler (3)**: Pipe làm nhiệm vụ chuẩn hóa (Transform) và kiểm tra (Validate) dữ liệu, sau đó mới thả vào Controller Handler.
> 5. **Interceptor (4 - Sau khi Handler xử lý)**: Sau khi Controller tính toán xong, Interceptor lại "chặn" một lần nữa để bọc/biến đổi Response.
> 6. **Exception Filter (5)**: Đứng như một chốt chặn an toàn cuối — nếu bất kỳ bước nào bên trên ném ra lỗi (Throw Exception), filter sẽ bắt lại và định dạng lỗi chuẩn chỉ thành Response.

Nếu nhìn dưới góc độ sơ đồ tuần tự (Sequence Diagram), luồng đi sẽ diễn ra như thế này:
...
```mermaid
graph LR
    classDef comp fill:#2b3440,stroke:#8b9eb3,stroke-width:2px,color:#fff;
    classDef global fill:#e1f5fe,stroke:#03a9f4,color:#000;
    classDef module fill:#e8f5e9,stroke:#4caf50,color:#000;
    
    Req([🌐 Request]) --> MW:::comp
    
    subgraph MW [1. Middleware - Tiền xử lý]
        direction LR
        GlobalMW(Global Scope):::global --> ModuleMW(Module Scope):::module
    end
    
    MW --> Next([Tiếp tục luồng...])
```
...
graph LR
    classDef comp fill:#2b3440,stroke:#8b9eb3,stroke-width:2px,color:#fff;
    classDef global fill:#e1f5fe,stroke:#03a9f4,color:#000;
    classDef controller fill:#e8f5e9,stroke:#4caf50,color:#000;
    classDef route fill:#fff3e0,stroke:#ff9800,color:#000;
    
    Req([Từ Middleware]) --> GD:::comp
    
    subgraph GD [2. Guard - Trạm gác phân quyền]
        direction LR
        GlobalGuard(Global Scope):::global --> ControllerGuard(Controller Scope):::controller --> RouteGuard(Method Scope):::route
    end
    
    GD -- Cho phép --> Next([Đến Interceptor...])
    GD -. Từ chối .-> Error([❌ 403 Forbidden])
```
...
```mermaid
graph TD
    classDef req fill:#2b3440,stroke:#8b9eb3,stroke-width:2px,color:#fff;
    classDef res fill:#4a90e2,stroke:#2b3440,stroke-width:2px,color:#fff;
    classDef global fill:#e1f5fe,stroke:#03a9f4,color:#000;
    classDef controller fill:#e8f5e9,stroke:#4caf50,color:#000;
    classDef route fill:#fff3e0,stroke:#ff9800,color:#000;
    
    Req([Từ Guard]) --> IntPre:::req
    
    subgraph IntPre [3. Interceptor Pre - Can thiệp trước]
        direction LR
        GlobalIntPre(Global Scope):::global --> ControllerIntPre(Controller Scope):::controller --> RouteIntPre(Method Scope):::route
    end
    
    IntPre --> Next([Đến Pipe & Handler...])
    HandlerReturn([Kết quả từ Handler]) --> IntPost:::res
    
    subgraph IntPost [5. Interceptor Post - Gói Response]
        direction RL
        RouteIntPost(Method Scope):::route --> ControllerIntPost(Controller Scope):::controller --> GlobalIntPost(Global Scope):::global
    end
    
    IntPost --> Res([✅ Response gửi về Client])
```
...
graph LR
    ...
    Req([Data từ Request]) --> PP:::comp
    
    subgraph PP [4. Pipe - Bộ lọc dữ liệu tinh]
        direction LR
        GlobalPipe(Global Scope):::global --> ControllerPipe(Controller Scope):::controller --> RoutePipe(Method Scope):::route --> ParamPipe(Tham số):::route
    end
    
    PP -- Dữ liệu chuẩn --> HTTPHandler((Xử lý HTTP <br/>Controller & Service))
    PP -. Data lỗi .-> Error([❌ 400 Bad Request])
```
...
```mermaid
graph RL
    classDef err fill:#f44336,stroke:#b71c1c,stroke-width:2px,color:#fff;
    classDef global fill:#e1f5fe,stroke:#03a9f4,color:#000;
    classDef controller fill:#e8f5e9,stroke:#4caf50,color:#000;
    classDef route fill:#fff3e0,stroke:#ff9800,color:#000;
    
    HTTPHandler((Lỗi phát sinh <br/>Throw Exception)) -.-> EF:::err
    
    subgraph EF [6. Exception Filter - Xử lý sự cố]
        direction RL
        RouteEF(Method/Route Scope):::route -.-> ControllerEF(Controller Scope):::controller -.-> GlobalEF(Global Scope):::global
    end
    
    EF -.-> ErrorResponse([❌ Định dạng chuẩn lỗi <br/>Response Thất bại])
```
