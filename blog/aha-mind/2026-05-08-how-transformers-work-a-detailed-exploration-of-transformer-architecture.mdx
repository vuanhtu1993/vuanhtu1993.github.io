---
slug: aha-mind-how-transformers-work-a-detailed-exploration-of-transformer-architecture
...
> Bài viết được biên dịch tự động bởi **Aha! Mind Socratic Engine**. Nguồn bài viết: [Link gốc](https://www.datacamp.com/tutorial/how-transformers-work)

<!-- truncate -->
## What Are Transformers?

Transformers were first developed to solve the problem of <Term definition="Quá trình biến đổi một chuỗi dữ liệu đầu vào thành một chuỗi đầu ra khác, thường thấy trong dịch thuật hoặc chuyển văn bản thành giọng nói. (Ví dụ: Giống như việc bạn đưa một xấp tiền Việt vào máy đổi tiền và nhận lại một xấp tiền Đô la tương ứng.)">sequence transduction</Term>, or neural machine translation, which means they are meant to solve any task that transforms an input sequence to an output sequence. This is why they are called “Transformers”.
...
Transformers are a current <Term definition="Cụm từ chỉ trình độ phát triển cao nhất, hiện đại nhất của một công nghệ hoặc kỹ thuật tại thời điểm hiện tại. (Ví dụ: Giống như chiếc iPhone đời mới nhất vừa ra mắt, hội tụ mọi công nghệ xịn nhất của Apple.)">state-of-the-art</Term> NLP model and are considered the evolution of the encoder-decoder architecture. However, while the encoder-decoder architecture relies mainly on Recurrent Neural Networks (RNNs) to extract sequential information, Transformers completely lack this recurrency.
...
_![The transformer architecture as a black box.](./assets/img-45065680.jpg)_

_Image by the author._
...
Their introduction has spurred a significant surge in the field, often referred to as Transformer AI. This revolutionary model laid the groundwork for subsequent breakthroughs in the realm of large language models, including BERT. By 2018, these developments were already being hailed as a <Term definition="Một sự kiện hoặc thời điểm đánh dấu sự thay đổi quan trọng, mang tính bước ngoặt lịch sử. (Ví dụ: Giống như việc phát minh ra Internet, nó chia lịch sử loài người thành hai kỷ nguyên: trước và sau khi có mạng.)">watershed moment</Term> in NLP.

In 2020, researchers at OpenAI announced [GPT-3](https://arxiv.org/abs/2005.14165). Within weeks, GPT-3's versatility was quickly demonstrated when people were using it to create poems, programs, songs, websites, and more, captivating the imagination of users globally. Observing this shift, Stanford scholars published a 2021 paper aptly terming these innovations "[foundation models](https://www.datacamp.com/blog/introduction-to-foundation-models)," underscoring their critical role in reshaping AI.

While <Term definition="Chỉ những công nghệ hoặc phần mềm thuộc quyền sở hữu riêng của một công ty, không được công khai mã nguồn. (Ví dụ: Giống như công thức bí mật của Coca-Cola, chỉ công ty mới biết và không ai khác được phép sao chép.)">proprietary</Term> models captured early public attention, a parallel open-source revolution rapidly <Term definition="Quá trình làm cho một công nghệ phức tạp hoặc đắt đỏ trở nên dễ tiếp cận và sử dụng rộng rãi cho mọi người. (Ví dụ: Giống như việc ngày xưa chỉ có nhiếp ảnh gia mới chụp được ảnh đẹp, nay ai có smartphone cũng làm được.)">democratized</Term> transformer technology. Platforms like [Hugging Face](https://www.datacamp.com/tracks/hugging-face-fundamentals) emerged as vital hubs for sharing models, but the landscape fundamentally changed in 2023 with the release of Meta's Llama family. This sparked an "open-weights" movement, proving developers no longer needed to rely on closed corporate ecosystems to build state-of-the-art AI.

Throughout 2024 and 2025, open-source development accelerated fiercely. Models like Meta's massive Llama 3.1 and subsequent Llama 4 series, alongside highly efficient architectures from startups like Mistral and powerful reasoning models from DeepSeek, proved that openly available models could match, and sometimes exceed, the performance of proprietary giants.

Meanwhile, closed-source models continued to evolve complex capabilities. GPT-4 in 2023, which introduced <Term definition="Khả năng của mô hình AI có thể hiểu và xử lý đồng thời nhiều loại dữ liệu khác nhau như văn bản, hình ảnh và âm thanh. (Ví dụ: Giống như một người vừa có thể nghe bạn nói, vừa nhìn cử chỉ của bạn để hiểu ý bạn muốn gì.)">multimodal</Term> capabilities, and [GPT-4o in 2024](https://www.datacamp.com/blog/what-is-gpt-4o), which unified text, vision, and audio processing in a single model.

Late 2024 marked another fundamental pivot across the industry with models like OpenAI's o1 series and DeepSeek's open R1, introducing an internal '<Term definition="Kỹ thuật giúp AI giải quyết vấn đề bằng cách chia nhỏ thành các bước suy luận logic trung gian thay vì đưa ra đáp án ngay lập tức. (Ví dụ: Giống như khi giải toán, bạn viết từng bước giải ra nháp thay vì chỉ ghi mỗi kết quả cuối cùng.)">chain-of-thought</Term>' to reason through complex logic and math before responding.

By late 2025 and into 2026, the landscape shifted entirely from conversational assistants to autonomous systems with the launch of the [GPT-5](https://www.datacamp.com/blog/gpt-5) family, bringing advanced multi-step planning, long-term memory, and robust <Term definition="Quy trình làm việc mà AI đóng vai trò như một 'đặc vụ' tự chủ, có khả năng lập kế hoạch và thực hiện nhiều bước để hoàn thành mục tiêu. (Ví dụ: Giống như việc bạn thuê một trợ lý ảo, bạn chỉ cần nói 'đặt vé máy bay', họ sẽ tự tìm chuyến, so sánh giá và thanh toán cho bạn.)">agentic workflows</Term> to enterprise infrastructure.

### The shift from RNN models like LSTM to Transformers for NLP problems

At the time of the Transformer model introduction, [Recurrent Neural Networks (RNNs)](https://www.datacamp.com/tutorial/tutorial-for-recurrent-neural-network) were the preferred approach for dealing with sequential data, which is characterized by a specific order in its input.

RNNs function similarly to a feed-forward neural network but process the input sequentially, one element at a time.

Transformers were inspired by the encoder-decoder architecture found in RNNs. However, Instead of using recurrence, the Transformer model is completely based on the <Term definition="Kỹ thuật cho phép mô hình AI tập trung vào những phần quan trọng nhất của dữ liệu đầu vào thay vì xử lý mọi thứ như nhau. (Ví dụ: Giống như khi bạn nhìn vào một bức ảnh đám đông, mắt bạn sẽ tự động tập trung vào khuôn mặt người thân thay vì nhìn tất cả mọi người.)">Attention mechanism</Term>.
...
If we start considering a Transformer for language translation as a simple <Term definition="Thuật ngữ chỉ một hệ thống mà ta chỉ biết đầu vào và đầu ra, còn cơ chế hoạt động bên trong cực kỳ phức tạp và khó hiểu. (Ví dụ: Giống như chiếc lò vi sóng, bạn bấm nút và thức ăn nóng, nhưng bạn không thực sự hiểu các sóng điện từ bên trong chạy thế nào.)">black box</Term>, it would take a sentence in one language, English for instance, as an input and output its translation in English.

_![The transformer architecture for language translating as a black box that translates from english to spanish.](./assets/img-a81c419b.jpg)_
...
_![The transformer architecture for language translating with two generic modules (Encoder and Decoder).](./assets/img-b3b88e6d.jpg)_

_Image by the author. Global structure of Encoder-Decoder._
...
_![The transformer architecture for language translating with two generic modules (Encoder and Decoder) repeated N times each.](./assets/img-8caec9ed.png)_

_Image by the author. Global structure of Encoder-Decoder. Multiple Layers._
...
_![The architecture of transformers' encoder.](./assets/img-18b49092.png)_

_Image by the author. Global structure of Encoders._

So let’s break its workflow into its most basic steps:

#### STEP 1 - Input <Term definition="Kỹ thuật chuyển đổi các từ ngữ hoặc ký hiệu thành các dãy số (vector) để máy tính có thể hiểu được ý nghĩa ngữ nghĩa. (Ví dụ: Giống như việc gán cho mỗi địa danh trên thế giới một tọa độ kinh độ và vĩ độ để định vị trên bản đồ.)">Embeddings</Term>
...
_![Encoder’s workflow. how input embedding works.](./assets/img-2f74b6ce.jpg)_

_Image by the author. Encoder’s workflow. Input embedding._

#### STEP 2 - <Term definition="Phương pháp thêm thông tin về thứ tự của các từ vào dữ liệu, giúp Transformer biết từ nào đứng trước, từ nào đứng sau. (Ví dụ: Giống như việc đánh số thứ tự cho các trang sách bị xé rời để bạn có thể xếp chúng lại đúng nội dung.)">Positional Encoding</Term>
...
_![Encoder’s workflow. How positional encoding works.](./assets/img-0f9db1f1.jpg)_

_Image by the author. Encoder’s workflow. Positional encoding._
...
*   A <Term definition="Cơ chế cho phép mô hình cùng lúc 'nhìn' vào dữ liệu từ nhiều góc độ khác nhau để hiểu ngữ cảnh sâu sắc hơn. (Ví dụ: Giống như một nhóm chuyên gia cùng đọc một hợp đồng: luật sư soi tính pháp lý, kế toán soi con số, giám đốc soi chiến lược.)">multi-headed attention</Term> mechanism.
*   A fully connected network.

Additionally, it incorporates <Term definition="Các đường kết nối tắt cho phép thông tin truyền thẳng qua các lớp mà không bị biến đổi quá nhiều, giúp huấn luyện mạng nơ-ron sâu hơn. (Ví dụ: Giống như việc xây thêm cầu vượt trên đường cao tốc để xe đi thẳng nhanh hơn thay vì phải dừng lại ở mọi ngã tư.)">residual connections</Term> around each sublayer, which are then followed by layer normalization.

_![Encoder’s workflow.  Stack of encoders layers.](./assets/img-f7f5a51e.jpg)_

_Image by the author. Encoder’s workflow. Stack of Encoder Layers_
...
![Encoder’s workflow.  Multi-head Attention mechanism.](./assets/img-c0f4132d.jpg)
...
_![Encoder’s workflow. Attention mechanism - Matrix Multiplication.](./assets/img-efbad7b7.jpg)_

_Image by the author. Encoder’s workflow. Attention mechanism - Matrix Multiplication._
...
_![Encoder’s workflow. Reducing attention scores.](./assets/img-3c86f2f9.jpg)_

_Image by the author. Encoder’s workflow. Reducing the attention scores._

##### Applying <Term definition="Hàm toán học dùng để chuyển đổi một dãy các con số thành các giá trị xác suất có tổng bằng 1. (Ví dụ: Giống như việc chia một chiếc bánh pizza cho các ứng viên dựa trên điểm số của họ, ai điểm cao nhất sẽ được miếng to nhất.)">Softmax</Term> to the Adjusted Scores
...
_![Encoder’s workflow. Applying Softmax to the adjusted scores.](./assets/img-bb44b138.jpg)_

_Image by the author. Encoder’s workflow. Softmax adjusted scores._
...
_![Encoder’s workflow. Combining Softmax Results with the value vector.](./assets/img-3d3d83bd.jpg)_

_Image by the author. Encoder’s workflow. Combining Softmax results with the value vector._
...
For a deeper understanding of the attention mechanism, see our tutorial on [multi-head attention in Transformers](https://www.datacamp.com/tutorial/multi-head-attention-transformers).

#### STEP 3.2 Normalization and Residual Connections

Each sub-layer in an encoder layer is followed by a normalization step. Also, each sub-layer output is added to its input (residual connection) to help mitigate the <Term definition="Hiện tượng tín hiệu học tập bị nhỏ dần và biến mất khi truyền qua quá nhiều lớp, khiến mô hình không thể học được nữa. (Ví dụ: Giống như trò chơi truyền tin, người đầu tiên nói rất to nhưng qua 100 người thì người cuối cùng chỉ nghe thấy tiếng thì thầm không rõ nghĩa.)">vanishing gradient</Term> problem, allowing deeper models. This process will be repeated after the Feed-Forward Neural Network too.

_![Encoder’s workflow. Normalization and residual connection after Multi-Head Attention.](./assets/img-7b793fd4.jpg)_

_Image by the author. Encoder’s workflow. Normalization and residual connection after Multi-Head Attention._

#### STEP 3.3 Feed-Forward Neural Network

The journey of the normalized residual output continues as it navigates through a <Term definition="Thao tác được áp dụng riêng biệt và độc lập cho từng vị trí hoặc từng phần tử trong một tập hợp dữ liệu. (Ví dụ: Giống như việc kiểm tra hộ chiếu của từng hành khách trong hàng đợi, người này xong mới đến người kia một cách độc lập.)">pointwise</Term> feed-forward network, a crucial phase for additional refinement.
...
_![Encoder’s workflow. Feed-Forward Neural Network sub-layer.](./assets/img-edf21bb4.jpg)_

_Image by the author. Encoder’s workflow. Feed-Forward Neural Network sub-layer._
...
_![Global structure of Encoders.](./assets/img-619681cb.jpg)_
...
It is important to notice that the decoder operates in an <Term definition="Cơ chế dự đoán phần tử tiếp theo dựa trên chính những phần tử đã được tạo ra trước đó trong chuỗi. (Ví dụ: Giống như việc bạn viết một câu, mỗi từ bạn viết ra đều dựa trên những từ bạn vừa viết trước đó để đảm bảo câu có nghĩa.)">autoregressive</Term> manner, kickstarting its process with a start token. It cleverly uses a list of previously generated outputs as its inputs, in tandem with the outputs from the encoder that are rich with attention information from the initial input.
...
_![Decoder’s workflow. First Multi-Head Attention Mask.](./assets/img-1ee95dce.jpg)_

_Image by the author. Decoder’s workflow. First Multi-Head Attention Mask._
...
_![Decoder’s workflow. Encoder-Decoder Attention.](./assets/img-759feb9d.png)_

_Image by the author. Decoder’s workflow. Encoder-Decoder Attention._
...
_![Decoder’s workflow. Transformer’s final output.](./assets/img-a9780260.jpg)_

_Image by the author. Decoder’s workflow. Transformer’s final output._
...
The final architecture looks like this (from the original paper):

![Original structure of Transformers.](./assets/img-405a469a.jpg)

_Image by the author. Original structure of Transformers._
...
If you are interested in further understanding the LaMDA models, you can get a better understanding with our [article on LaMDA](https://www.datacamp.com/blog/what-is-lamda).
...
Claude, developed by Anthropic, is a family of Transformer-based AI assistants designed with a focus on safety and helpfulness. Claude uses Constitutional AI (CAI), a training approach where the model is guided by a set of principles to produce helpful, harmless, and honest responses.

Claude 3 (released 2024) introduced three model tiers, Haiku, Sonnet, and Opus, offering different trade-offs between speed and capability. The models excel at nuanced reasoning, following complex instructions, and maintaining context over long conversations (up to 200K tokens).

In 2025 and 2026, Anthropic launched the Claude 4 models, with the most recent, [Opus 4.6](https://www.datacamp.com/blog/claude-opus-4-6) and [Sonnet 4.6](https://www.datacamp.com/blog/claude-sonnet-4-6), topping many LLM benchmarks. 

### Other Variations

The landscape of <Term definition="Các mô hình AI khổng lồ được huấn luyện trên lượng dữ liệu cực lớn, làm nền tảng để phát triển các ứng dụng chuyên biệt khác. (Ví dụ: Giống như bộ khung của một chiếc xe hơi, từ đó bạn có thể tùy biến thành xe tải, xe đua hay xe gia đình.)">foundation models</Term>, particularly transformer models, is rapidly expanding. A study identified over 50 significant transformer models, while [the Stanford group evaluated 30 of them](https://arxiv.org/pdf/2302.07730.pdf), acknowledging the field's fast-paced growth. NLP Cloud, an innovative startup part of NVIDIA's Inception program, utilizes around 25 large language models commercially for various sectors like airlines and pharmacies.
...
Since the original 2017 architecture, Transformers have evolved significantly to address limitations and expand to new domains.

### Vision Transformers (ViT)

[Vision Transformers](https://www.datacamp.com/tutorial/vision-transformers) apply the self-attention mechanism to images by dividing them into patches and treating each patch as a token. This approach has proven highly effective for image classification, object detection, and image generation, often outperforming traditional CNNs on large datasets.

### Multimodal Transformers

Modern models like GPT-5, Gemini 3, and Llama 4 process multiple input types (text, images, audio, video) within a single architecture. These multimodal Transformers use unified embedding spaces and cross-attention mechanisms to reason across different modalities simultaneously.

### Efficient Transformers

The quadratic complexity of self-attention limits context length. Several innovations address this:

*   **[Flash Attention](https://www.datacamp.com/blog/flash-attention):** Optimizes memory access patterns to reduce GPU memory usage and speed up training by 2-4x
*   **Rotary Position Embeddings (RoPE):** Encodes position information through rotation matrices, enabling better extrapolation to longer sequences
*   **Linear attention variants:** Linformer, Performer, and Longformer reduce complexity to O(n) for processing very long documents
*   **Mixture of Experts (MoE):** Activates only a subset of parameters per token, enabling larger models with similar compute costs
...
In the world of Neural Networks, two prominent structures are usually compared to Transformers. Each of them offers distinct benefits and challenges, tailored for specific types of data processing. RNNs, which has already appeared throughout the article multiple times, andConvolutional Layers.
...
*   **Quadratic complexity:** Self-attention scales as O(n²) with sequence length, making very long contexts computationally expensive
*   **Memory requirements:** Large models require significant GPU memory, limiting deployment options
*   **Training data requirements:** Transformers typically need massive datasets to achieve strong performance
*   **Lack of explicit reasoning:** While powerful pattern matchers, Transformers don't perform symbolic reasoning and can hallucinate facts
*   **Position encoding limitations:** Standard positional encodings struggle to generalize to sequence lengths not seen during training

Research continues to address these limitations through architectural innovations like [Flash Attention](https://www.datacamp.com/blog/flash-attention), [mixture-of-experts](https://www.datacamp.com/blog/mixture-of-experts-moe), and [retrieval-augmented generation (RAG)](https://www.datacamp.com/blog/what-is-retrieval-augmented-generation-rag).

## Conclusion

In conclusion, Transformers have emerged as a monumental breakthrough in artificial intelligence and natural language processing (NLP).
