...
crawled_at: "2026-06-25T07:00:36.664Z"
---
...
![Three zones: A, B, C. Showing a hard navigation between routes from different zones, and soft navigations between routes within the same zone.](https://res.cloudinary.com/dv3vzmogk/image/upload/v1782370841/aha-mind/docs-crawler/nextjs.org/image_pf1wol.png)![Three zones: A, B, C. Showing a hard navigation between routes from different zones, and soft navigations between routes within the same zone.](https://res.cloudinary.com/dv3vzmogk/image/upload/v1782370841/aha-mind/docs-crawler/nextjs.org/image_orxioi.png)
...
Links to paths in a different zone should use an `a` tag instead of the Next.js [`<Link>`](https://nextjs.org/docs/app/api-reference/components/link) component. This is because Next.js will try to prefetch and soft navigate to any relative path in `<Link>` component, which will not work across zones.
...
When using [Server Actions](https://nextjs.org/docs/app/getting-started/mutating-data) with Multi-Zones, you must explicitly allow the user-facing origin since your user facing domain may serve multiple applications. In your `next.config.js` file, add the following lines:
