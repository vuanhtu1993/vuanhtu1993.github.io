---
title: "Guides: PPR Platform Guide"
source_url: "https://nextjs.org/docs/app/guides/ppr-platform-guide"
crawled_at: "2026-06-25T07:00:54.319Z"
---
...
-   A **static HTML shell** containing all the content that can be prerendered, with [Suspense](https://nextjs.org/docs/app/guides/streaming#what-is-streaming) fallbacks where dynamic content will appear.
...
## Storing PPR Artifacts[](#storing-ppr-artifacts)
...
These must be stored and updated atomically. When a PPR route is revalidated (via [time-based](https://nextjs.org/docs/app/guides/incremental-static-regeneration) or [on-demand revalidation](https://nextjs.org/docs/app/api-reference/functions/revalidateTag)), Next.js regenerates both the shell and the postponed state together. Serving a new shell with an old postponed state, or vice versa, will produce incorrect dynamic content.

Use [`requestMeta.onCacheEntryV2`](https://nextjs.org/docs/app/api-reference/adapters/implementing-ppr-in-an-adapter) in your adapter to observe cache updates and propagate them to your storage backend.
...
## CDN Shell + Origin Compute[](#cdn-shell--origin-compute)
...
## The Resume Protocol[](#the-resume-protocol)
...
In the [adapter output](https://nextjs.org/docs/app/api-reference/adapters/output-types), PPR routes are identified by `renderingMode: 'PARTIALLY_STATIC'` in the prerenders array. Iterate `outputs.prerenders` to find these entries and read `fallback.postponedState`.

`pprChain.headers` contains the headers needed for the resume protocol: `{ 'next-resume': '1' }`.

For detailed implementation with code examples, see [Implementing PPR in an Adapter](https://nextjs.org/docs/app/api-reference/adapters/implementing-ppr-in-an-adapter).
...
For the complete adapter API reference and implementation examples, see the [Deployment Adapter API](https://nextjs.org/docs/app/api-reference/config/next-config-js/adapterPath).
