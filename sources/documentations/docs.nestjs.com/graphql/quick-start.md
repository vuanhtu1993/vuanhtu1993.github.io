...
crawled_at: "2026-06-23T15:21:43.422Z"
---
...
You can also build your own dedicated driver (read more on that [here](https://docs.nestjs.com/graphql/other-features#creating-a-custom-driver)).

#### Installation[#](https://docs.nestjs.com/graphql/quick-start#installation)
...
> **Warning**`@nestjs/graphql@>=9` and `@nestjs/apollo^10` packages are compatible with **Apollo v3** (check out Apollo Server 3 [migration guide](https://www.apollographql.com/docs/apollo-server/migration/) for more details), while `@nestjs/graphql@^8` only supports **Apollo v2** (e.g., `apollo-server-express@2.x.x` package).

#### Overview[#](https://docs.nestjs.com/graphql/quick-start#overview)
...
> **Hint** In the following chapters, we'll be integrating the `@nestjs/apollo` package. If you want to use `mercurius` package instead, navigate to [this section](https://docs.nestjs.com/graphql/quick-start#mercurius-integration).
...
> **Hint** For `mercurius` integration, you should be using the `MercuriusDriver` and `MercuriusDriverConfig` instead. Both are exported from the `@nestjs/mercurius` package.
...
![](https://res.cloudinary.com/dv3vzmogk/image/upload/v1782228102/aha-mind/docs-crawler/docs.nestjs.com/playground_lv4pef.png)

> **Note**`@nestjs/mercurius` integration does not ship with the built-in GraphQL Playground integration. Instead, you can use [GraphiQL](https://github.com/graphql/graphiql) (set `graphiql: true`).

> **Warning** Update (04/14/2025): The default Apollo playground has been deprecated and will be removed in the next major release. Instead, you can use [GraphiQL](https://github.com/graphql/graphiql), just set `graphiql: true` in the `GraphQLModule` configuration, as shown below:
> ```
> 
> GraphQLModule.forRoot<ApolloDriverConfig>({
>   driver: ApolloDriver,
...
> ```
> 
> If your application uses [subscriptions](https://docs.nestjs.com/graphql/subscriptions), be sure to use `graphql-ws`, as `subscriptions-transport-ws` isn't supported by GraphiQL.
...
#### Example[#](https://docs.nestjs.com/graphql/quick-start#example)
...
> **Hint** You can compile the script beforehand (e.g., with `tsc`) and use `node` to execute it.
...
#### Apollo Sandbox[#](https://docs.nestjs.com/graphql/quick-start#apollo-sandbox)
...
#### Example[#](https://docs.nestjs.com/graphql/quick-start#example-1)
...
#### Accessing generated schema[#](https://docs.nestjs.com/graphql/quick-start#accessing-generated-schema)
...
> **Hint** You must call the `GraphQLSchemaHost#schema` getter after the application has been initialized (after the `onModuleInit` hook has been triggered by either the `app.listen()` or `app.init()` method).

#### Async configuration[#](https://docs.nestjs.com/graphql/quick-start#async-configuration)
...
#### Mercurius integration[#](https://docs.nestjs.com/graphql/quick-start#mercurius-integration)

Instead of using Apollo, Fastify users (read more [here](https://docs.nestjs.com/techniques/performance)) can alternatively use the `@nestjs/mercurius` driver.
...
> **Hint** Once the application is running, open your browser and navigate to `http://localhost:3000/graphiql`. You should see the [GraphQL IDE](https://github.com/graphql/graphiql).
...
#### Multiple endpoints[#](https://docs.nestjs.com/graphql/quick-start#multiple-endpoints)
...
> **Warning** If you use the `@apollo/server` with `@as-integrations/fastify` package with multiple GraphQL endpoints in a single application, make sure to enable the `disableHealthCheck` setting in the `GraphQLModule` configuration.

#### Third-party integrations[#](https://docs.nestjs.com/graphql/quick-start#third-party-integrations)

-   [GraphQL Yoga](https://github.com/dotansimha/graphql-yoga)

#### Example[#](https://docs.nestjs.com/graphql/quick-start#example-2)
