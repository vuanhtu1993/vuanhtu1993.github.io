...
crawled_at: "2026-06-23T15:23:12.381Z"
---
...
#### Creating a strategy[#](https://docs.nestjs.com/microservices/custom-transport#creating-a-strategy)
...
> **Warning** Please, note we won't be implementing a fully-featured Google Cloud Pub/Sub server in this chapter as this would require diving into transporter specific technical details.
...
> **Hint** If we used the `@EventPattern` decorator, you would see the same output, but with the `isEventHandler` property set to `true`.
...
When using a `CustomTransportStrategy` with [Interceptors](https://docs.nestjs.com/interceptors) the handlers are wrapped into RxJS streams. This means that you need to subscribe to them in order to execute the streams underlying logic (e.g. continue into the controller logic after an interceptor has been executed).
...
#### Client proxy[#](https://docs.nestjs.com/microservices/custom-transport#client-proxy)
...
> **Hint** Again, implementing a fully-featured client class compatible with all `@nestjs/microservices` features (e.g., streaming) requires a good understanding of communication techniques used by the framework. To learn more, check out this [article](https://dev.to/nestjs/part-4-basic-client-component-16f9).
...
> **Warning** Please, note we won't be implementing a fully-featured Google Cloud Pub/Sub client in this chapter as this would require diving into transporter specific technical details.
...
#### Message serialization[#](https://docs.nestjs.com/microservices/custom-transport#message-serialization)
...
> **hint** This is the class itself being passed to `customClass`, not an instance of the class. Nest will create the instance under the hood for you, and will pass any options given to the `options` property to the new `ClientProxy`.
