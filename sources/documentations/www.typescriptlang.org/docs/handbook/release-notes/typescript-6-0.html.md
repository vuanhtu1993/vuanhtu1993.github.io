---
title: "Documentation - TypeScript 6.0"
source_url: "https://www.typescriptlang.org/docs/handbook/release-notes/typescript-6-0.html"
crawled_at: "2026-07-02T07:39:07.682Z"
---

## [](#less-context-sensitivity-on-this-less-functions)Less Context-Sensitivity on `this`\-less Functions
...
Here, TypeScript can infer the type of `y` in the `consume` function based on the inferred `T` from the `produce` function, regardless of the order of the properties. But what about if these functions were written using _method syntax_ instead of arrow function syntax?
...
TypeScript 6.0 takes this into account when it decides if a function is contextually sensitive or not. If `this` is never actually _used_ in a function, then it is not considered contextually sensitive. That means these functions will be seen as higher-priority when it comes to type inference, and all of our examples above now work!
...
One minor annoyance with this feature has been that developers always had to write _something_ after the `#` when specifying a subpath import. Here, we used `root`, but it is a bit useless since there is no directory we’re mapping over other than `./dist/`
...
## [](#combining---moduleresolution-bundler-with---module-commonjs)Combining `--moduleResolution bundler` with `--module commonjs`
...
## [](#the---stabletypeordering-flag)The `--stableTypeOrdering` Flag
...
If you encounter a type error using `--stableTypeOrdering`, this is typically due to inference differences. The previous inference without `--stableTypeOrdering` _happened_ to work based on the current ordering of types in your program. To help with this, you’ll often benefit from providing an explicit type somewhere. Often, this will be a type argument
...
## [](#es2025-option-for-target-and-lib)`es2025` option for `target` and `lib`

TypeScript 6.0 adds support for the `es2025` option for both `target` and `lib`. While there are no new JavaScript language features in ES2025, this new target adds new types for built-in APIs (e.g. `RegExp.escape`), and moves a few declarations from `esnext` into `es2025` (e.g. `Promise.try`, `Iterator` methods, and `Set` methods). Work to enable [the new target](https://github.com/microsoft/TypeScript/pull/63046) was contributed thanks to [Kenta Moriuchi](https://github.com/petamoriken).

## [](#new-types-for-temporal)New Types for `Temporal`

The long-awaited [Temporal proposal](https://github.com/tc39/proposal-temporal) has reached stage 4 and will be part of a future ECMAScript standard. TypeScript 6.0 now includes built-in types for the Temporal API, so you can start using it in your TypeScript code today via `--target esnext` or `"lib": ["esnext"]` (or the more-granular `esnext.temporal`).
...
Temporal is already usable in several runtimes, and with stage 4 status it is now officially part of the JavaScript language. [Documentation on the Temporal APIs is available on MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Temporal).
...
## [](#new-types-for-upsert-methods-aka-getorinsert)New Types for “upsert” Methods (a.k.a. `getOrInsert`)
...
When constructing some literal string to match within a regular expression, it is important to escape special regular expression characters like `*`, `+`, `?`, `(`, `)`, etc. The [RegExp Escaping ECMAScript proposal](https://github.com/tc39/proposal-regex-escaping) has reached stage 4, and introduces a new `RegExp.escape` function that takes care of this for you.
...
## [](#the-dom-lib-now-contains-domiterable-and-domasynciterable)The `dom` lib Now Contains `dom.iterable` and `dom.asynciterable`
...
## [](#breaking-changes-and-deprecations-in-typescript-60)Breaking Changes and Deprecations in TypeScript 6.0
...
### [](#up-front-adjustments)Up-Front Adjustments
...
### [](#simple-default-changes)Simple Default Changes
...
### [](#rootdir-now-defaults-to-)`rootDir` now defaults to `.`
...
### [](#types-now-defaults-to-)`types` now defaults to `[]`
...
In a sense, the `types` value previously defaulted to “enumerate everything in `node_modules/@types`”. This can be _very_ expensive, as a normal repository setup these days might transitively pull in hundreds of `@types` packages, especially in multi-project workspaces with flattened `node_modules`. Modern projects almost always need only `@types/node`, `@types/jest`, or a handful of other common global-affecting packages.
...
### [](#deprecated-target-es5)Deprecated: `target: es5`
...
### [](#deprecated---downleveliteration)Deprecated: `--downlevelIteration`
...
### [](#deprecated---moduleresolution-node-aka---moduleresolution-node10)Deprecated: `--moduleResolution node` (a.k.a. `--moduleResolution node10`)
...
### [](#deprecated-amd-umd-and-systemjs-values-of-module)Deprecated: `amd`, `umd`, and `systemjs` values of `module`
...
### [](#deprecated---baseurl)Deprecated: `--baseUrl`
...
### [](#deprecated---moduleresolution-classic)Deprecated: `--moduleResolution classic`
...
### [](#deprecated---esmoduleinterop-false-and---allowsyntheticdefaultimports-false)Deprecated: `--esModuleInterop false` and `--allowSyntheticDefaultImports false`
...
### [](#deprecated---alwaysstrict-false)Deprecated: `--alwaysStrict false`
...
### [](#deprecated-outfile)Deprecated: `outFile`
...
### [](#deprecated-legacy-module-syntax-for-namespaces)Deprecated: legacy `module` Syntax for namespaces
...
### [](#deprecated-asserts-keyword-on-imports)Deprecated: `asserts` Keyword on Imports
...
### [](#deprecated-no-default-lib-directives)Deprecated: `no-default-lib` Directives
...
### [](#specifying-command-line-files-when-tsconfigjson-exists-is-now-an-error)Specifying Command-Line Files When `tsconfig.json` Exists is Now an Error
...
## [](#preparing-for-typescript-70)Preparing for TypeScript 7.0
