---
title: "Documentation - TypeScript 5.0"
source_url: "https://www.typescriptlang.org/docs/handbook/release-notes/typescript-5-0.html"
crawled_at: "2026-07-02T07:39:58.433Z"
---
...
While this is a limitation, it does help make some issues more obvious. For example, it’s very common to forget to set the [`type` field in `package.json`](https://nodejs.org/api/packages.html#type) under `--module node16`. As a result, developers would start writing CommonJS modules instead of ES modules without realizing it, giving surprising lookup rules and JavaScript output. This new flag ensures that you’re intentional about the file type you’re using because the syntax is intentionally different.
...
![A set of case statements generated through auto-completion based on literal types.](https://res.cloudinary.com/dv3vzmogk/image/upload/v1782977999/aha-mind/docs-crawler/www.typescriptlang.org/switchCaseSnippets-5-0_1_zdogbd.gif)
...
![Chart of build/run times and package size of TypeScript 5.0 relative to TypeScript 4.9: material-ui docs build time: 89%; Playwright build time: 88%; tsc startup time: 87%; tsc build time: 87%; Outlook Web build time: 82%; VS Code build time: 80%; typescript Package Size: 59%](https://res.cloudinary.com/dv3vzmogk/image/upload/v1782977999/aha-mind/docs-crawler/www.typescriptlang.org/speed-and-size-5-0-rc_vyjiix.png)
