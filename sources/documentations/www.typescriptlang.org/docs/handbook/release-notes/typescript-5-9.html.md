...
source_url: "https://www.typescriptlang.org/docs/handbook/release-notes/typescript-5-9.html"
crawled_at: "2026-07-02T07:39:14.061Z"
---

## [](#minimal-and-updated-tsc---init)Minimal and Updated `tsc --init`
...
Note that when you write `import defer`, the module and its dependencies are fully loaded and ready for execution. That means that the module will need to exist, and will be loaded from the file system or a network resource. The key difference between a regular `import` and `import defer` is that _the execution of statements and declarations_ is deferred until you access a property of the imported namespace.
...
## [](#support-for---module-node20)Support for `--module node20`
...
## [](#summary-descriptions-in-dom-apis)Summary Descriptions in DOM APIs
...
## [](#expandable-hovers-preview)Expandable Hovers (Preview)

_Quick Info_ (also called “editor tooltips” and “hovers”) can be very useful for peeking at variables to see their types, or at type aliases to see what they actually refer to. Still, it’s common for people to want to _go deeper_ and get details from whatever’s displayed within the quick info tooltip. For example, if we hover our mouse over the parameter `options` in the following example:
...
![Tooltip for a parameter declared as options which just shows options: Options.](https://res.cloudinary.com/dv3vzmogk/image/upload/v1782977955/aha-mind/docs-crawler/www.typescriptlang.org/bare-hover-5.8-01_stulci.png)

Do we really need to jump to the definition of the type `Options` just to see what members this value has?

Previously, that was actually the case. To help here, TypeScript 5.9 is now previewing a feature called _expandable hovers_, or “quick info verbosity”. If you use an editor like VS Code, you’ll now see a `+` and `-` button on the left of these hover tooltips. Clicking on the `+` button will expand out types more deeply, while clicking on the `-` button will collapse to the last view.
...
## [](#configurable-maximum-hover-length)Configurable Maximum Hover Length
...
### [](#cache-instantiations-on-mappers)Cache Instantiations on Mappers
...
### [](#avoiding-closure-creation-in-fileordirectoryexistsusingsource)Avoiding Closure Creation in `fileOrDirectoryExistsUsingSource`
...
## [](#type-argument-inference-changes)Type Argument Inference Changes
