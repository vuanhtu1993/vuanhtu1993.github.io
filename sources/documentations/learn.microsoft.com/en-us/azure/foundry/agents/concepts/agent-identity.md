---
title: "Agent identity concepts in Microsoft Foundry - Microsoft Foundry"
source_url: "https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/agent-identity?source=recommendations"
crawled_at: "2026-06-27T11:31:06.363Z"
---

An _agent identity_ is a specialized identity type in [Microsoft Entra ID](https://learn.microsoft.com/en-us/entra/fundamentals/what-is-entra) that's designed specifically for AI agents. It provides a standardized framework for governing, authenticating, and authorizing AI agents across Microsoft services. This framework enables agents to securely access resources, interact with users, and communicate with other systems.
...
For Foundry-specific RBAC roles and scopes, see [Azure role-based access control in Foundry](https://learn.microsoft.com/en-us/azure/foundry/concepts/rbac-foundry).
...
The Agent ID platform framework introduces formal _agent identities_ and _agent identity blueprints_ in Microsoft Entra ID to represent AI agents. You can use this framework to securely communicate with AI agents. This framework also enables those AI agents to securely communicate with web services, other AI agents, and various systems.
...
An agent identity blueprint serves as the reusable, governing template from which all associated agent identities are created. It corresponds to a _kind_, _type_, or _class_ of agents. It acts as the management object for all agent identity instances of that class.
...
The managed identity authenticates the _blueprint_ to Entra ID. It doesn't directly access the downstream resource. The agent identity — not the managed identity — is the principal that requires RBAC role assignments on the target resource.
...
[![Screenshot of the JSON view in the Azure portal displaying an agent identity blueprint and agent identity details for a Foundry project.](https://res.cloudinary.com/dv3vzmogk/image/upload/v1782559865/aha-mind/docs-crawler/learn.microsoft.com/azure-agent-identity-json-view_r3whx5.png)](https://learn.microsoft.com/en-us/azure/foundry/agents/media/agent-identity/azure-agent-identity-json-view.png#lightbox)
...
azd does not configure Container Registry, Application Insights, or custom resource permissions. For production deployments and the complete permission requirements for Hosted agents, see [Hosted agent permissions reference](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/hosted-agent-permissions).
...
When you [publish an agent](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/agent-applications), you must reassign RBAC permissions to the new agent identity for any resources that the agent needs to access. This reassignment ensures that the published agent maintains appropriate access while operating under its distinct identity.
...
For comprehensive details about all permissions involved in Hosted agent deployment, including Azure Container Registry, Application Insights, and multi-resource RBAC configurations, see [Hosted agent permissions reference](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/hosted-agent-permissions).

Currently, the tools that support authentication with an agent identity are:

-   **Model Context Protocol (MCP)**: Use your agent's identity to authenticate with MCP servers that support agent identity authentication. For details, see [Model Context Protocol](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/model-context-protocol) and [MCP server authentication](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/mcp-authentication).
-   **Agent-to-Agent (A2A)**: Enable secure communication between agents by using agent identities. For details, see [Agent-to-Agent tool](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/agent-to-agent) and [Agent2Agent (A2A) authentication](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/agent-to-agent-authentication).
...
-   [Set up MCP server authentication](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/mcp-authentication#use-agent-identity-authentication)
-   [Agent2Agent (A2A) authentication](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/agent-to-agent-authentication)
...
-   **Missing role assignments**: Ensure the agent identity has the required RBAC role on the target resource. For Foundry roles and scopes, see [Azure role-based access control in Foundry](https://learn.microsoft.com/en-us/azure/foundry/concepts/rbac-foundry).
...
-   [Model Context Protocol](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/model-context-protocol)
-   [Agent-to-Agent tool](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/agent-to-agent)
...
[![Screenshot of the Microsoft Entra admin center that shows the tab for agent identities with an inventory of all agents in the tenant.](https://res.cloudinary.com/dv3vzmogk/image/upload/v1782559865/aha-mind/docs-crawler/learn.microsoft.com/entra-admin-center-agent-identities_nppkrg.png)](https://learn.microsoft.com/en-us/azure/foundry/agents/media/agent-identity/entra-admin-center-agent-identities.png#lightbox)
