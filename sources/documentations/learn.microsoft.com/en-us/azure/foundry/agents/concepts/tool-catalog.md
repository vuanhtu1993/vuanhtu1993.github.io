---
title: "Agent tools overview for Microsoft Foundry Agent Service - Microsoft Foundry"
source_url: "https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/tool-catalog?source=recommendations"
crawled_at: "2026-06-27T11:31:10.720Z"
---
...
A _tool_ is a capability that an agent can invoke during a conversation to perform a specific task. When an agent receives a user message, the Foundry model powering the agent decides whether to call a tool based on the agent's instructions and the available tool definitions. The agent sends the tool request, your application or the service executes it, and the result flows back into the conversation so the agent can continue with accurate, up-to-date information.
...
-   **[Web search](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/web-search)** — Add web search to your agent. The agent retrieves real-time information from the public web and returns answers with inline citations. This method is the recommended way to add web grounding. For advanced scenarios such as market-specific filtering, see [Grounding with Bing tools](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/bing-tools) and the [web grounding overview](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/web-overview).
-   **[Code Interpreter](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/code-interpreter)** — Let agents write and run Python code in a sandboxed environment for data analysis, math, and chart generation.
-   **[File Search](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/file-search)** — Augment agents with knowledge from uploaded files or proprietary documents by using vector search.
-   **[Function calling](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/function-calling)** — Define custom functions that the agent can call. Your application executes the function and returns the result.
...
-   **[Model Context Protocol (MCP)](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/model-context-protocol)** — Connect your agent to tools hosted on an MCP server endpoint. It's best for tools shared across multiple agents or maintained by a different team.
-   **[Agent-to-Agent (A2A) (preview)](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/agent-to-agent)** — Connect your agent to other agents through A2A-compatible endpoints for cross-agent communication.
-   **[OpenAPI tool](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/openapi)** — Connect your agent to external HTTP APIs by using an OpenAPI 3.0 or 3.1 specification.

For the complete list of custom tool options, see [All custom tools](#all-custom-tools).

A _toolbox_ is a curated bundle of tools - such as web search, Azure AI Search, code interpreter, file search, MCP servers, and OpenAPI tools - that you configure once and expose as a single MCP-compatible endpoint. Instead of attaching each tool individually to every agent definition, define the collection in a toolbox and connect any agent to the toolbox endpoint. Because a toolbox exposes an MCP-compatible endpoint, any MCP-capable runtime can consume it - including agents built with Microsoft Agent Framework, LangGraph, GitHub Copilot SDK, and custom code.

Toolboxes support versioning. Create multiple versions, test a new version against the version-specific endpoint, and then promote it to default when it's ready. Agents that connect to the Toolbox consumer endpoint automatically receive the promoted default version without code changes. For details, see [Manage toolbox versions](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/toolbox#promote-a-version-to-default).

Manage authentication for tools in a toolbox centrally. The toolbox handles credential injection, token refresh, and policy enforcement at runtime by using Microsoft Entra ID and OAuth, so consuming agents don't need to manage credentials for each tool individually. For authentication details, see [Set up MCP server authentication](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/mcp-authentication).

For setup steps, see [Create and use a Foundry Toolbox](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/toolbox).
...
By default, tool configurations such as file IDs, vector store IDs, and MCP server endpoints are fixed when you create the agent. _Structured inputs_ in tool properties allow you to override these values at runtime without creating a new agent version.
...
Most built-in tools, such as Code Interpreter and File Search, automatically authenticate through Foundry Agent Service and require no extra configuration. Tools that connect to external data sources, such as Azure AI Search or SharePoint, use the [connections](https://learn.microsoft.com/en-us/azure/foundry/how-to/connections-add) you configure in your Foundry project.
...
For detailed setup steps for all methods, see [Set up MCP server authentication](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/mcp-authentication).
...
For managed identity configuration, see [Connect agents to OpenAPI tools](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/openapi).

Tip

Treat all credentials as secrets. Only provide the minimum required headers, don't include credentials in prompts, and review the provider's data handling practices. For governance controls such as rate limits and IP restrictions on MCP tools, see [Govern MCP tools by using an AI gateway](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/governance).

The following table lists all built-in tools available in Foundry Agent Service.

| Tool | Description |
| --- | --- |
| [Web search](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/web-search) | Retrieve real-time information from the public web and return answers with inline citations. |
| [Code Interpreter](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/code-interpreter) | Write and run Python code in a sandboxed environment. |
| [Custom Code Interpreter (preview)](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/custom-code-interpreter) | Customize the code interpreter's resources, Python packages, and Container Apps environment. |
| [File Search](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/file-search) | Augment agents with knowledge from uploaded files or proprietary documents. |
| [Azure AI Search](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/ai-search) | Ground agents with data from an existing Azure AI Search index. |
| [Azure Functions](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/azure-functions) | Enable agents to call your Azure Functions to perform custom actions and retrieve dynamic data. |
| [Function calling](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/function-calling) | Define custom functions that the agent can call. Your app executes the function and returns the result. |
| [Image Generation (preview)](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/image-generation) | Generate images as part of conversations and workflows. |
| [Browser Automation (preview)](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/browser-automation) | Perform browser tasks through natural language prompts. |
| [Computer Use (preview)](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/computer-use) | Interact with computer systems through their user interfaces. |
| [Microsoft Fabric (preview)](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/fabric) | Connect to a [Microsoft Fabric data agent](https://go.microsoft.com/fwlink/?linkid=2312815) for data analysis. |
| [SharePoint (preview)](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/sharepoint) | Chat with private documents stored in SharePoint. |

The following table lists all custom tool options for connecting your own capabilities to an agent.

| Tool | Description |
| --- | --- |
| [Model Context Protocol (MCP)](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/model-context-protocol) | Connect your agent to tools hosted on an MCP server endpoint. |
| [OpenAPI tool](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/openapi) | Connect your agent to external APIs using an OpenAPI 3.0 or 3.1 specification. |
| [Agent-to-Agent (A2A) (preview)](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/agent-to-agent) | Connect your agent to other agents through A2A-compatible endpoints. |
| [Toolbox (preview)](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/toolbox) | Bundle multiple tools into a single MCP endpoint for reuse across agents. |
...
In the Foundry portal, go to your project and select **Build** > **Tools** to open Foundry Tools. From there, you can browse the tool catalog, configure tools, and add them to agents or workflows. If you need tools that are only visible within your organization, create a [private tool catalog](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/private-tool-catalog).

To explore tools while you build, use the Agents playground. For more information, see [Microsoft Foundry Playgrounds](https://learn.microsoft.com/en-us/azure/foundry/concepts/concept-playgrounds).
...
**Local MCP server**: You host the server yourself, then connect it to Foundry by providing its endpoint. To build and register your own server, see [Build and register an MCP server](https://learn.microsoft.com/en-us/azure/foundry/mcp/build-your-own-mcp-server). To connect an MCP endpoint to an agent, see [Connect to MCP servers](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/model-context-protocol).
...
| Registry | **Public**: Public remote and local MCP servers in the catalog.  
**Logic Apps connectors**: Azure Logic Apps connectors that you convert to remote MCP servers for use in a private tool catalog. |
...
[![Screenshot of a tool details page in the Foundry portal showing configuration and setup information.](https://res.cloudinary.com/dv3vzmogk/image/upload/v1782559870/aha-mind/docs-crawler/learn.microsoft.com/tool-example_fexwt4.png)](https://learn.microsoft.com/en-us/azure/foundry/agents/media/tool-catalog/tool-example.png#lightbox)
...
For the latest model and region support details across tools, see [Best practices for using tools in Foundry Agent Service](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/tool-best-practice).
...
-   **Your agent doesn't call a tool**: Use the validation guidance in [Best practices for using tools in Foundry Agent Service](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/tool-best-practice).
...
-   [Create and use a Foundry Toolbox (preview)](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/toolbox)
...
-   [Create a private tool catalog](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/private-tool-catalog)
-   [Build and register an MCP server](https://learn.microsoft.com/en-us/azure/foundry/mcp/build-your-own-mcp-server)
-   [Foundry MCP Server best practices and security guidance](https://learn.microsoft.com/en-us/azure/foundry/mcp/security-best-practices)
