---
title: "Networking options for Foundry Agent Service - Microsoft Foundry"
source_url: "https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/networking-options"
crawled_at: "2026-06-27T11:30:15.994Z"
---
...
| --- | --- | --- |
| Fastest path to a working agent, no isolation | Public, Microsoft-managed storage | [Deploy your first hosted agent quickstart](https://learn.microsoft.com/en-us/azure/foundry/agents/quickstarts/quickstart-hosted-agent) (Azure Developer CLI or VS Code) |
...
| Full isolation with no public egress, you control the network | BYO virtual network (network-secured standard) | [`15-private-network-standard-agent-setup`](https://github.com/microsoft-foundry/foundry-samples/tree/main/infrastructure/infrastructure-setup-bicep/15-private-network-standard-agent-setup) |
...
When you choose BYO virtual network, plan for these requirements before you deploy. The [setup how-to](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/virtual-networks) and the [deep dive](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/agents-networking-deep-dive) cover them in full.
...
-   **Bring-your-own data resources.** Standard, network-secured setups require your own Azure Storage, Azure AI Search, and Azure Cosmos DB so all agent data stays in your tenant.
-   **Private endpoints and Private DNS zones** for the Foundry account and each data resource, so name resolution stays inside the virtual network.
...
For a topology diagram of the BYO virtual network option - the delegated subnet, hosted-agent Micro VMs, and private endpoints to your data resources - see [Deep dive into Foundry Agent Service networking](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/agents-networking-deep-dive#network-architecture-overview).
...
For the IP-allocation model, concurrent-session limits, and sizing math, see [Deep dive into Foundry Agent Service networking](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/agents-networking-deep-dive).
...
1.  **Deploy** the option with the [setup how-to](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/virtual-networks) or the template from the table.
2.  **Validate** the deployment: confirm subnet delegation, that public access is disabled, and that endpoints resolve to private IP addresses from inside the virtual network. See [Verify the deployment](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/virtual-networks#verify-the-deployment).
3.  **Troubleshoot** any deployment or connectivity errors with the [troubleshooting guide](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/virtual-networks#troubleshooting-guide).
...
-   [Configure network isolation](https://learn.microsoft.com/en-us/azure/foundry/how-to/configure-private-link)
