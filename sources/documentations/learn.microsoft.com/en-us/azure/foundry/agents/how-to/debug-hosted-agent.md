---
title: "Debug a hosted agent - Microsoft Foundry"
source_url: "https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/debug-hosted-agent"
crawled_at: "2026-06-27T11:28:59.015Z"
---
...
-   For deployment and log issues, a deployed hosted agent. To deploy one, see [Deploy a hosted agent](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/deploy-hosted-agent).
...
For a structured health report, run `azd ai agent doctor`. For more information, see [Diagnose a project with agent doctor](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/agent-doctor).
...
For the full resolution order and when each source wins, see [Understand azd project context](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/cli-project-context).
...
2.  For `timer` triggers, verify `--at` is in the future and has not already fired.
3.  For `recurring` triggers, verify the `--cron` expression is valid and that the `--time-zone` is what you expected.
4.  For `github-issue` triggers, verify that the `--connection-id` resolves to a healthy connection and that the GitHub repository and `--issue-event` match the events the repository emits.
5.  For `custom` triggers, verify the `--provider`, `--event-name`, and `--parameters` scope matches events the provider is publishing.
...
-   [Monitor hosted agent logs with the Azure Developer CLI](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/monitor-hosted-agent-logs) for deeper log inspection options.
-   [Test a hosted agent](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/test-hosted-agent) to prevent issues with structured testing.
-   [Diagnose a project with agent doctor](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/agent-doctor) for a structured project health report.
