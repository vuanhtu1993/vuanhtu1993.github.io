---
title: "Build a voice agent with hosted agents (preview) - Microsoft Foundry"
source_url: "https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/build-voice-agent"
crawled_at: "2026-06-27T11:26:29.626Z"
---
...
For background on hosted agents and the available protocols, see [What are hosted agents?](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/hosted-agents). For general container packaging and deployment steps, see [Deploy a hosted agent](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/deploy-hosted-agent).
...
For request/response style invocations, continue to use the HTTP [`/invocations`](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/deploy-hosted-agent#protocol-libraries) or `/responses` routes. A single container can expose all three protocols at the same time.
...
2.  **Exchange.** Frames flow in both directions until either side initiates close.
3.  **Close.** Either side sends a Close frame; the peer echoes a Close frame and shuts down its send side.
4.  **Abnormal close.** If the underlying TCP connection drops, the peer observes close code `1006` with no Close frame.
...
| --- | --- |
| [Microsoft Voice Live](https://learn.microsoft.com/en-us/azure/ai-services/speech-service/voice-live) | Real-time voice interaction with built-in STT, LLM, and TTS. Supports cascaded pipelines and speech-to-speech models such as GPT-4o Realtime. |
...
For the full deployment flow (build, push, RBAC, polling for status), see [Deploy a hosted agent](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/deploy-hosted-agent).
...
2.  The browser sends signaling messages as text frames, such as an ICE configuration request, SDP offer, and ICE candidates.
3.  The container creates or updates its WebRTC peer connection and responds with an SDP answer and candidate status messages.
4.  Audio media flows over the WebRTC peer connection. The WebSocket remains available for signaling, lifecycle events, and application control messages.
5.  Your application provides the TURN relay configuration and handles any WebRTC-specific reconnection or resume behavior.
...
-   **Tools.** Access Foundry-managed tools (Web Search, File Search, Code Interpreter, Azure AI Search, custom MCP servers, A2A) through the Toolbox MCP endpoint in your Foundry project. See [Curate intent-based toolbox in Foundry](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/toolbox).

The `/invocations_ws` upgrade and per-connection lifecycle should be instrumented with distributed trace spans, following the [OpenTelemetry GenAI conventions](https://learn.microsoft.com/en-us/azure/foundry/observability/concepts/trace-agent-concept) used by the HTTP `/invocations` protocol.
...
-   [Configure virtual networks](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/virtual-networks)
