---
title: "Automate agents with routines (preview) - Microsoft Foundry"
source_url: "https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/use-routines?pivots=foundry-portal"
crawled_at: "2026-06-27T11:30:53.955Z"
---
...
A _routine_ is a named automation rule that triggers an agent on a schedule or at a specific time. You define what fires the routine (the _trigger_) and what agent to invoke (the _action_). Foundry queues the invocation, runs the agent, and stores a run record you can inspect later.
...
-   An active [Microsoft Foundry project](https://learn.microsoft.com/en-us/azure/foundry/how-to/create-projects) with at least one agent deployed.
...
-   Install the [Azure CLI](https://learn.microsoft.com/en-us/cli/azure/install-azure-cli) to acquire access tokens.

-   Install the [Azure Developer CLI](https://learn.microsoft.com/en-us/azure/developer/azure-developer-cli/install-azd) (`azd` 1.23.13 or later).
...
2.  In the left navigation, select **Routines**.
    
3.  Select **\+ New routine**.
    
4.  Enter a **Name** for the routine, for example `daily-summary`.
    
5.  Select an **Agent** from the dropdown.
    
6.  Enter a **Prompt** for the agent to run on each invocation.
    
7.  Under **Trigger**, set **Type** to **Recurring schedule**, then choose a **Frequency** (**Daily** or **Weekly**) and a **Time**.
    
8.  Select **Create & start**.
    
    [![Screenshot of the New routine dialog with the Recurring schedule trigger type selected, showing Frequency set to Daily and Time set to 9:00 AM.](https://res.cloudinary.com/dv3vzmogk/image/upload/v1782559853/aha-mind/docs-crawler/learn.microsoft.com/routine-recurring-schedule_iirxfh.png)](https://learn.microsoft.com/en-us/azure/foundry/agents/media/routines/routine-recurring-schedule.png#lightbox)
...
2.  Under **Trigger**, set **Type** to **One-time schedule**.
    
3.  Under **Run at**, pick the date and time when the routine should fire.
    
4.  Select **Create & start**.
    
    ![Screenshot of the New routine dialog with the One-time schedule trigger type selected, showing a date picker and a time picker under Run at.](https://res.cloudinary.com/dv3vzmogk/image/upload/v1782559853/aha-mind/docs-crawler/learn.microsoft.com/routine-one-time-schedule_e0qjnu.png)
...
2.  Select **Routines** in the left navigation.
    
3.  Find the routine and select it.
    
4.  Select **Pause** in the top right to pause the routine, or select **Resume** to re-enable a paused routine.
    
    [![Screenshot of a routine detail page showing the Pause button in the top right, the overflow menu with Test run, Edit, and Delete options, and the table of past runs below.](https://res.cloudinary.com/dv3vzmogk/image/upload/v1782559853/aha-mind/docs-crawler/learn.microsoft.com/routine-detail-actions_uubyjx.png)](https://learn.microsoft.com/en-us/azure/foundry/agents/media/routines/routine-detail-actions.png#lightbox)
...
2.  Select **Test run** (either the button on the right or the entry in the overflow menu next to **Pause**).
...
1.  Open the routine in [Microsoft Foundry](https://ai.azure.com/).
2.  Past runs are listed in the table on the routine detail page. Each row shows the response ID, when the run was triggered, its duration, and its state (for example **Completed** or **Failed**).
3.  Use the **Last day**, **7D**, **1M**, or **Custom** range controls above the table to filter the time window.
4.  Select a response ID to open the full run, including the response and any error details.
...
1.  Open the routine in [Microsoft Foundry](https://ai.azure.com/).
2.  Select **Edit**.
3.  Change the trigger or action settings.
4.  Select **Save**.
...
-   [Create and configure agents](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/configure-agent)
-   [Model Context Protocol (MCP)](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/model-context-protocol)
-   [Use toolboxes with agents](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/toolbox)
