---
title: "Agent applications in Microsoft Foundry - Microsoft Foundry"
source_url: "https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/agent-applications?source=recommendations"
crawled_at: "2026-06-27T11:29:47.915Z"
---
...
Because the identity changes, **permissions don't transfer automatically**. When you publish an agent, you must reassign RBAC permissions to the new agent identity for any resources that the agent needs to access. If you skip this step, tool calls that work during development fail with authorization errors once the agent is published.
...
-   [Foundry Project Manager role](https://learn.microsoft.com/en-us/azure/foundry/concepts/rbac-foundry) on the Foundry resource scope to publish agents
-   [Foundry User role](https://learn.microsoft.com/en-us/azure/foundry/concepts/rbac-foundry) on the Agent Application scope to chat with a published agent using the Responses API protocol
...
![Diagram illustrating how Foundry projects organize agent versions, applications, and deployments, highlighting governance and RBAC roles.](https://res.cloudinary.com/dv3vzmogk/image/upload/v1782559787/aha-mind/docs-crawler/learn.microsoft.com/azure-agent-identity-overview_hvnqrb.png)

A Foundry **project** is a work organization concept that groups related resources such as agents, files, and indexes. An **agent** represents a composable unit — defined by its instructions, model, and tools. An **agent version** captures a specific immutable snapshot of an agent. Every time you make changes to your agent, such as updating the prompt or adding tools, a new agent version is created. Each agent version is exposed under the Foundry project, where developers with project access can create, run, and test it.
...
Each Agent Application acts as a routing table to specific agent deployments. Currently, an Agent Application supports one active deployment, directing 100% of the traffic received by the application’s endpoint to that deployment. When you publish a new agent version to an existing application, 100% of the traffic received by the application’s endpoint is directed to the deployment referencing the new agent version.

![Diagram of an Agent Application routing traffic to a deployment running a specific agent version, showing a stable entry point and traffic flow.](https://res.cloudinary.com/dv3vzmogk/image/upload/v1782559787/aha-mind/docs-crawler/learn.microsoft.com/agent-application-routing-diagram_cfw539.png)
...
For applications, this endpoint is exposed at:
...
This limitation means the client must store conversation history for multi-turn conversations.
...
API key authentication isn't supported for invoking Agent Applications. Use Microsoft Entra ID (Azure RBAC) to authorize callers.
...
Use the `accessToken` value as the `Authorization: Bearer <token>` header in the following requests.

If you want to capture only the token value (for example, for use in a script), use:
...
For a full property reference and an infrastructure-as-code (Bicep) example for Agent Applications, see the Azure Resource Manager template reference for [Microsoft.CognitiveServices/accounts/projects/applications](https://learn.microsoft.com/en-us/azure/templates/microsoft.cognitiveservices/accounts/projects/applications?pivots=deployment-language-bicep).
...
For a full property reference and an infrastructure-as-code (Bicep) example for Agent deployments, see the Azure Resource Manager template reference for [Microsoft.CognitiveServices/accounts/projects/applications/agentDeployments](https://learn.microsoft.com/en-us/azure/templates/microsoft.cognitiveservices/accounts/projects/applications/agentdeployments?pivots=deployment-language-bicep).
...
To run the following commands, you need the [Azure CLI](https://learn.microsoft.com/en-us/cli/azure/install-azure-cli).
...
2.  Call the Agent Application endpoint (Responses protocol).
...
Agent Application RBAC is managed through Azure Resource Manager, not through the Entra agent identity. The Entra agent identity that the published agent receives is for the agent's _own_ outbound calls to tools and resources. To control who can _invoke_ the published agent, assign Azure RBAC roles on the Agent Application ARM resource by using the Azure portal, Azure CLI, or REST API.

For more information about Azure RBAC, see [Role-based access control for Microsoft Foundry](https://learn.microsoft.com/en-us/azure/foundry/concepts/rbac-foundry)
...
To use your Agent Application in Microsoft 365 Copilot and Teams, see [Publish agents to Microsoft 365 Copilot and Microsoft Teams](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/publish-copilot).

To publish your agent as an autopilot, see [Publish an agent as an autopilot in Agent 365](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/agent-365)

To invoke your Agent Application using the Responses API protocol, you need:

-   [Foundry User role](https://learn.microsoft.com/en-us/azure/foundry/concepts/rbac-foundry) on the Agent Application scope
-   The `openai` and `azure-identity` packages installed and authenticated as described in [Prepare your development environment](https://learn.microsoft.com/en-us/azure/foundry/how-to/develop/install-cli-sdk)
...
| `authorizationPolicy` | Defines how users are allowed to auth to the app. If not specified, this field is set by default | object | ✅ |
...
| `deploymentId` | A system-generated unique identifier for each distinct lifetime of a deployment with a given resource identifier. | string | ❌ (read only) |
...
**What is the pricing model for a published agent? Is the cost based on a consumption model, or does the client incur charges simply because the application resource (endpoint) has underlying infrastructure deployed once the agent is published?**
...
-   Learn how to [publish agents to Microsoft 365 Copilot and Microsoft Teams](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/publish-copilot)
