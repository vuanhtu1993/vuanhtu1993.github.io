---
title: "Optimize agent instructions, skills, tools, and models in Foundry Agent Service (preview) - Microsoft Foundry"
source_url: "https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/optimize-agent-targets?source=recommendations"
crawled_at: "2026-06-27T11:35:43.395Z"
---
...
-   A model deployed for evaluation (for example, `gpt-4.1-mini`) and an optimization model from the [supported list](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/agent-optimizer-overview#models) (for example, `gpt-5.1`)
-   Your agent is [optimizer-ready](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/make-agent-optimizer-ready) (calls `load_config()`)
...
In most cases, you don't need to specify the agent name explicitly. The CLI uses your current `azd` environment and your project's local `agent.yaml` file to detect the local agent. Use the `--agent` flag when you have multiple agents in your project or want to override the default:
...
2.  **Instruction generation.** The optimizer analyzes the baseline scores and generates alternative system prompts designed to improve weak areas while maintaining strong areas.
3.  **Candidate evaluation.** The optimizer injects each candidate instruction set into your agent through the `OPTIMIZATION_CANDIDATE_ID` environment variable and evaluates it against the same dataset.
4.  **Ranking.** The optimizer ranks candidates by composite score and marks the best candidate with ★.
...
When `dataset` includes `local_uri` and doesn't include `name` or `version`, the optimizer treats the dataset as a local file.
...
The `max_candidates` option sets the expected number of candidate instruction sets for the optimization run. The optimizer typically returns after it reaches that count, unless the run stops early because of an error or another stopping condition.
...
Always verify that your eval model is deployed in the project before you run optimization.

You must specify the deployed optimization model that generates candidate configurations. Supported optimization models include the gpt-5 family and DeepSeek models. For the current list and model roles, see [Models](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/agent-optimizer-overview#models).

Specify the optimization model in your config file:
...
For more details on how these models are used, see [Models](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/agent-optimizer-overview#models).
...
2.  **Skill improvement.** The optimizer analyzes weak areas and refines skill definitions. A skill is a named capability with:
    
    -   **Name**: For example, `"step_by_step_reasoning"`
    -   **Description**: What the skill does and when to use it
    -   **Body**: Implementation details or procedure
3.  **Injection.** The agent loads improved skills through `load_config()`, which makes them available to your agent's instruction set.
...
4.  **Evaluation.** The optimizer evaluates the agent with improved skills against the dataset.
...
2.  **Tool analysis.** The optimizer identifies tool calls that fail or produce suboptimal results and analyzes the root cause—unclear descriptions, missing parameters, or ambiguous naming.
3.  **Description refinement.** The optimizer generates improved tool definitions with clearer descriptions, better parameter documentation, and more precise function names.
4.  **Evaluation.** The optimizer evaluates the agent with improved tool definitions against the dataset.
...
The optimizer evaluates your agent across multiple model deployments to find the best quality-to-cost trade-off. Each model runs against the same dataset, so you can compare results directly. Model optimization activates when you specify model candidates in `optimization_config.model_search_space`.
...
options:
  ...
  max_candidates: 5
  optimization_config:
    # Target model candidates for optimization.
    model_search_space:
      - gpt-4.1
      - gpt-4.1-mini
      - gpt-4o
...
Each model listed under `optimization_config.model_search_space` must be deployed in your Foundry project.
...
options:
  ...
  optimization_model: gpt-5.1
  max_candidates: 5
  # Free-form per-attribute overrides sent to the optimization service.
  # Each key is an attribute name; the value is a JSON-encoded payload.
  optimization_config:
    # Target model candidates for optimization.
    model_search_space:
      - gpt-4.1
...
After optimization completes, review the results table. The ★ marks the best candidate. For the results table columns, scoring details, score-improvement thresholds, and the portal view, see [Understand optimization results](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/agent-optimizer-overview#understand-optimization-results).
