---
title: "Connect to MCP Server Endpoints for agents - Microsoft Foundry"
source_url: "https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/model-context-protocol?tabs=prompt-agents&pivots=python"
crawled_at: "2026-06-27T11:33:37.950Z"
---
...
-   The latest SDK package for your language. The .NET SDK is currently in preview. For installation details, see the [quickstart](https://learn.microsoft.com/en-us/azure/foundry/quickstarts/get-started-code).
...
-   **Private endpoints**: Connect to MCP servers that aren't exposed to the public internet. Private MCP requires [Standard Agent Setup with private networking](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/virtual-networks) and a dedicated MCP subnet within your virtual network.
...
For details about tool support in network-isolated environments, see [Agent tools with network isolation](https://learn.microsoft.com/en-us/azure/foundry/how-to/configure-private-link#agent-tools-with-network-isolation).
...
To learn about supported authentication options, including key-based, Microsoft Entra identities, and OAuth identity passthrough, see [MCP server authentication](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/mcp-authentication).
...
For general guidance on tool usage, see [Best practices for using tools in Microsoft Foundry Agent Service](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/tool-best-practice).
...
Use the following code sample to create an agent and call the function. The .NET SDK is currently in preview. See the [quickstart](https://learn.microsoft.com/en-us/azure/foundry/quickstarts/get-started-code) for details.

The following example shows how to use the GitHub MCP server as a tool for an agent. Select **Prompt Agents** to use the Azure AI Projects SDK to create a server-side prompt agent, or **Hosted Agents** to use the Agent Framework [`FoundryChatClient`](https://learn.microsoft.com/en-us/azure/foundry/agents/quickstarts/responses-api) to build an ephemeral, in-process agent.
...
For details on the MCP tool schema and approval items, see [OpenAI.MCPTool](https://learn.microsoft.com/en-us/azure/foundry/reference/foundry-project-rest-preview#openaimcptool) and the MCP approval item types in the REST reference.
...
**Toolbox versioning**: Foundry Toolboxes support versioning, so you can iterate on a new version without affecting production agents. Use the **consumer endpoint** (`{project_endpoint}/toolboxes/{name}/mcp?api-version=v1`) for production agents - it always serves the promoted default version. Use the **version-specific endpoint** (`{project_endpoint}/toolboxes/{name}/versions/{version}/mcp?api-version=v1`) to test before promoting. Keep `server_label` unique per agent, even when switching Toolbox versions. For details, see [Promote a version to default](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/toolbox#promote-a-version-to-default).

Some MCP servers expose tools that take longer than the standard synchronous timeout to return a result. To support these operations, run the agent in [background mode](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/runtime-components#run-an-agent-in-background-mode). Background mode runs the response asynchronously, so the MCP tool call can continue without holding an open connection, and you poll for the response status until it completes. This approach lets MCP tool calls exceed the 100-second non-streaming timeout described in [Known limitations](#known-limitations).
...
-   **Private MCP requires Standard Agent Setup**: Private MCP server connectivity is only available with [Standard Agent Setup with private networking](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/virtual-networks) (BYO VNet). Basic agent setup doesn't support private MCP endpoints.
...
-   [Get started with agents using code](https://learn.microsoft.com/en-us/azure/foundry/quickstarts/get-started-code)
...
-   [MCP tool REST reference](https://learn.microsoft.com/en-us/azure/foundry/reference/foundry-project-rest-preview#openaimcptool)
