---
title: "Migrate hosted agents to the refreshed public preview - Microsoft Foundry"
source_url: "https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/migrate-hosted-agent-preview?source=recommendations"
crawled_at: "2026-06-27T11:32:30.614Z"
---
...
-   [Azure Developer CLI](https://learn.microsoft.com/en-us/azure/developer/azure-developer-cli/install-azd) version 1.23.0 or later with the updated Foundry agents extension:
...
2.  **Update API, CLI, and SDK calls** — Remove retired CLI commands, update SDK methods, and switch to the dedicated agent endpoint. See [Removed APIs](#removed-apis), [CLI command mapping](#cli-command-mapping), [SDK method changes](#sdk-method-changes), and [Agent invocation changes](#agent-invocation-changes).
3.  **Update identity and RBAC** — Grant downstream resource access to the agent's dedicated Entra identity. See [Identity and RBAC changes](#identity-and-rbac-changes).
4.  **Update Azure Developer CLI tooling** — Install the latest `azd` Foundry agents extension and update `agent.yaml`. See [Azure Developer CLI changes](#azure-developer-cli-changes).
5.  **Redeploy and verify** — Build your container image, deploy using `azd up` or the SDK, and confirm the version reaches `active` status.
...
Choose your protocol based on your agent's interaction pattern. See [What are Hosted agents — Protocols](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/hosted-agents#protocols-responses-invocations-and-invocations-websocket) for guidance on which protocol to use.
