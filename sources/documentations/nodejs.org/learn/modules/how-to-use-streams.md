---
title: "How To Use Streams | Node.js Learn"
source_url: "https://nodejs.org/learn/modules/how-to-use-streams"
crawled_at: "2026-07-20T08:10:34.716Z"
---
...
This approach differs from loading the entire dataset into memory at once. Streams process data in chunks, significantly reducing memory usage. All streams in Node.js inherit from the [`EventEmitter`](https://nodejs.org/api/events.html#class-eventemitter) class, allowing them to emit events at various stages of data processing. These streams can be readable, writable, or both, providing flexibility for different data-handling scenarios.
...
They achieve this by emitting events at key stages. These events include signals for received data ([`data`](https://nodejs.org/api/stream.html#stream_event_data) event) and the stream's completion ([`end`](https://nodejs.org/api/stream.html#event-end) event). Developers can listen to these events and execute custom logic accordingly. This event-driven nature makes streams highly efficient for the processing of data from external sources.
...
### [Streams 1 (Classic)](#streams-1-classic)
...
[`Readable`](https://nodejs.org/api/stream.html#stream_readable_streams) is the class that we use to sequentially read a source of data. Typical examples of `Readable` streams in Node.js API are [`fs.ReadStream`](https://nodejs.org/api/fs.html#class-fsreadstream) when reading files, [`http.IncomingMessage`](https://nodejs.org/api/http.html#class-httpincomingmessage) when reading HTTP requests, and [`process.stdin`](https://nodejs.org/api/process.html#processstdin) when reading from the standard input.

#### [Key Methods and Events](#key-methods-and-events)

A readable stream operates with several core methods and events that allow fine control over data handling:

-   **[`on('data')`](https://nodejs.org/api/stream.html#stream_event_data)**: This event is triggered whenever data is available from the stream. It is very fast, as the stream pushes data as quickly as it can handle, making it suitable for high-throughput scenarios.
-   **[`on('end')`](https://nodejs.org/api/stream.html#event-end)**: Emitted when there is no more data to read from the stream. It signifies the completion of data delivery. This event is only fired when all the data from the stream has been consumed.
-   **[`on('readable')`](https://nodejs.org/api/stream.html#event-readable)**: This event is triggered when there is data available to read from the stream or when the end of the stream has been reached. It allows for more controlled data reading when needed.
-   **[`on('close')`](https://nodejs.org/api/stream.html#event-close_1)**: This event is emitted when the stream and its underlying resources have been closed and indicates that no more events will be emitted.
-   **[`on('error')`](https://nodejs.org/api/stream.html#event-error_1)**: This event can be emitted at any point, signaling that there was an error processing. A handler for this event can be used to avoid uncaught exceptions.

A demonstration of the use of these events can be seen in the following sections.

#### [Basic Readable Stream](#basic-readable-stream)
...
In this code, the `MyStream` class extends Readable and overrides the [`_read()`](https://nodejs.org/api/stream.html#readable_readsize) method to push a string ":-)" to the internal buffer. After pushing the string five times, it signals the end of the stream by pushing `null`. The [`on('data')`](https://nodejs.org/api/stream.html#stream_event_data) event handler logs each chunk to the console as it is received.

#### [Advanced Control with the readable Event](#advanced-control-with-the-readable-event)
...
[`Writable`](https://nodejs.org/api/stream.html#stream_writable_streams) streams are useful for creating files, uploading data, or any task that involves sequentially outputting data. While readable streams provide the source of data, writable streams in Node.js act as the destination for your data. Typical examples of writable streams in the Node.js API are [`fs.WriteStream`](https://nodejs.org/api/fs.html#class-fswritestream) , [`process.stdout`](https://nodejs.org/api/process.html#processstdout) , and [`process.stderr`](https://nodejs.org/api/process.html#processstderr) .

#### [Key Methods and Events in Writable Streams](#key-methods-and-events-in-writable-streams)

-   **[`.write()`](https://nodejs.org/api/stream.html#stream_writable_write_chunk_encoding_callback)**: This method is used to write a chunk of data to the stream. It handles the data by buffering it up to a defined limit (highWaterMark), and returns a boolean indicating whether more data can be written immediately.
-   **[`.end()`](https://nodejs.org/api/stream.html#writableendchunk-encoding-callback)**: This method signals the end of the data writing process. It signals the stream to complete the write operation and potentially perform any necessary cleanup.

#### [Creating a Writable](#creating-a-writable)
...
In this code, `MyStream` is a custom [`Writable`](https://nodejs.org/api/stream.html#stream_writable_streams) stream with a buffer capacity ([`highWaterMark`](https://nodejs.org/api/stream.html#stream_buffering)) of 10 bytes. It overrides the [`_write`](https://nodejs.org/api/stream.html#writable_writechunk-encoding-callback) method to convert data to uppercase before writing it out.

The loop attempts to write hello ten times to the stream. If the buffer fills up (`waitDrain` becomes `true`), it waits for a [`drain`](https://nodejs.org/api/stream.html#stream_event_drain) event before continuing, ensuring we do not overwhelm the stream's buffer.
...
[`Duplex`](https://nodejs.org/api/stream.html#stream_duplex_and_transform_streams) streams implement both the readable and writable interfaces.

#### [Key Methods and Events in Duplex Streams](#key-methods-and-events-in-duplex-streams)
...
[`Transform`](https://nodejs.org/api/stream.html#stream_duplex_and_transform_streams) streams are duplex streams, where the output is computed based on the input. As the name suggests, they are usually used between a readable and a writable stream to transform the data as it passes through.

#### [Key Methods and Events in Transform Streams](#key-methods-and-events-in-transform-streams)

Apart from all the methods and events in Duplex Streams, there is:

-   **[`_transform`](https://nodejs.org/api/stream.html#transform_transformchunk-encoding-callback)**: This function is called internally to handle the flow of data between the readable and writable parts. This MUST NOT be called by application code.
...
The [`.pipe()`](https://nodejs.org/docs/latest/api/stream.html#stream_readable_pipe_destination_options) method concatenates one readable stream to a writable (or transform) stream. Although this seems like a simple way to achieve our goal, it delegates all error handling to the programmer, making it difficult to get it right.
...
To avoid the pitfalls and low-level complexity of the `.pipe()` method, in most cases, it is recommended to use the [`pipeline()`](https://nodejs.org/api/stream.html#stream_stream_pipeline_streams_callback) method. This method is a safer and more robust way to pipe streams together, handling errors and cleanup automatically.
...
The [`pipeline()`](https://nodejs.org/api/stream.html#stream_stream_pipeline_streams_callback) method also has an [`async pipeline()`](https://nodejs.org/api/stream.html#streampipelinesource-transforms-destination-options) version, which doesn’t accept a callback but instead returns a promise that is rejected if the pipeline fails.
...
#### [Benefits of Using Async Iterators with Streams](#benefits-of-using-async-iterators-with-streams)
...
By default, streams can work with strings, [`Buffer`](https://nodejs.org/api/buffer.html), [`TypedArray`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/TypedArray), or [`DataView`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/DataView). If an arbitrary value different from these (e.g., an object) is pushed into a stream, a `TypeError` will be thrown. However, it is possible to work with objects by setting the `objectMode` option to `true`. This allows the stream to work with any JavaScript value, except for `null`, which is used to signal the end of the stream. This means you can `push` and `read` any value in a readable stream, and `write` any value in a writable stream.
...
For a deeper understanding of backpressure, check the [`backpressure guide`](https://nodejs.org/learn/modules/backpressuring-in-streams).

## [Streams vs Web streams](#streams-vs-web-streams)

The stream concept is not exclusive to Node.js. In fact, Node.js has a different implementation of the stream concept called [`Web Streams`](https://nodejs.org/api/webstreams.html), which implements the [`WHATWG Streams Standard`](https://streams.spec.whatwg.org/). Although the concepts behind them are similar, it is important to be aware that they have different APIs and are not directly compatible.

[`Web Streams`](https://nodejs.org/api/webstreams.html) implement the [`ReadableStream`](https://nodejs.org/api/webstreams.html#class-readablestream), [`WritableStream`](https://nodejs.org/api/webstreams.html#class-writablestream), and [`TransformStream`](https://nodejs.org/api/webstreams.html#class-transformstream) classes, which are homologous to Node.js's [`Readable`](https://nodejs.org/api/stream.html#stream_readable_streams), [`Writable`](https://nodejs.org/api/stream.html#stream_writable_streams), and [`Transform`](https://nodejs.org/api/stream.html#stream_duplex_and_transform_streams) streams.

### [Interoperability of streams and Web Streams](#interoperability-of-streams-and-web-streams)
...
The following example in the [`Duplex`](https://nodejs.org/api/stream.html#stream_duplex_and_transform_streams) class demonstrates how to work with both readable and writable streams converted to Web Streams:
...
Be aware that the fetch body is a `ReadableStream<Uint8Array>`, and therefore a [`TextDecoderStream`](https://developer.mozilla.org/en-US/docs/Web/API/TextDecoderStream) is needed to work with chunks as strings.

This work is derived from content published by [Matteo Collina](https://github.com/mcollina) in [Platformatic's Blog](https://blog.platformatic.dev/a-guide-to-reading-and-writing-nodejs-streams).
