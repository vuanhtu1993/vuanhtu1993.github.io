---
title: "Publishing a package | Node.js Learn"
source_url: "https://nodejs.org/learn/modules/publishing-a-package"
crawled_at: "2026-07-20T08:10:40.117Z"
---

All the provided `package.json` configurations (not specifically marked “does not work”) work in Node.js 12.22.x (v12 latest, the oldest supported line) and 17.2.0 current latest at the time[1](#user-content-fn-1), and for grins, with webpack 5.53.0 and 5.63.0 respectively. These are available: [JakobJingleheimer/nodejs-module-config-examples](https://github.com/JakobJingleheimer/nodejs-module-config-examples).
...
| ESM source code using `import` | CJS: consumers `require()` your package (and you use top-level `await`) | [ESM source with only CJS distribution](#esm-source-with-only-cjs-distribution) |
| ESM source code using `import` | CJS & ESM: consumers either `require()` or `import` your package | [ESM source and both CJS & ESM distribution](#esm-source-and-both-cjs-amp-esm-distribution) |

### [CJS source and distribution](#cjs-source-and-distribution)
...
### [ESM source and distribution](#esm-source-and-distribution)
...
### [CJS source and only ESM distribution](#cjs-source-and-only-esm-distribution)
...
### [CJS source and both CJS & ESM distribution](#-esm-distribution)

In order to _directly_ supply both audiences (so that your distribution works "natively" in either), you have a few options:

#### [Attach named exports directly onto `exports`](#attach-named-exports-directly-onto-exports)
...
#### [Use a simple ESM wrapper](#use-a-simple-esm-wrapper)
...
#### [Two full distributions](#two-full-distributions)
...
### [ESM source with only CJS distribution](#esm-source-with-only-cjs-distribution)
...
💡 Using `"type": "module"`[2](#user-content-fn-2) paired with the `.cjs` file extension (for commonjs files) yields best results. For more information on why, see [Down the rabbit-hole](#down-the-rabbit-hole) and [Gotchas](#gotchas) below.
...
### [ESM source and both CJS & ESM distribution](#-esm-distribution)
...
#### [Publish only a CJS distribution with property exports](#publish-only-a-cjs-distribution-with-property-exports)
...
#### [Publish a CJS distribution with an ESM wrapper](#publish-a-cjs-distribution-with-an-esm-wrapper)
...
#### [Publish both full CJS & ESM distributions](#-esm-distributions)
...
##### [Mark the whole package as ESM and specifically mark the CJS exports as CJS via the `.cjs` file extension](#mark-the-whole-package-as-esm-and-specifically-mark-the-cjs-exports-as-cjs-via-the-cjs-file-extension)
...
##### [Use the `.mjs` (or equivalent) file extension for all source code files](#use-the-mjs-or-equivalent-file-extension-for-all-source-code-files)
...
#### [Node.js before 12.22.x](#nodejs-before-1222x)
...
1.  There was a bug in Node.js v13.0–13.6 where `packageJson.exports["."]` had to be an array with verbose config options as the first item (as an object) and the “default” as the second item (as a string). See [nodejs/modules#446](https://github.com/nodejs/modules/issues/446). [↩](#user-content-fnref-1)
    
2.  The `"type"` field in package.json changes what the `.js` file extension means, similar to to an [HTML script element’s type attribute](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/script#attr-type). [↩](#user-content-fnref-2) [↩2](#user-content-fnref-2-2) [↩3](#user-content-fnref-2-3) [↩4](#user-content-fnref-2-4)
