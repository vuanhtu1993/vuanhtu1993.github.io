...
The **Functional API** and the [**Graph API**](https://docs.langchain.com/oss/javascript/langgraph/graph-api) can be used together in the same application as they share the same underlying runtime.
...
The **Functional API** uses the same streaming mechanism as the **Graph API**. Please read the [**streaming guide**](https://docs.langchain.com/oss/javascript/langgraph/streaming) section for more details. Example of using the streaming API to stream both updates and custom data.
...
The functional API supports [human-in-the-loop](https://docs.langchain.com/oss/javascript/langgraph/interrupts) workflows using the [`interrupt`](https://reference.langchain.com/javascript/langchain-langgraph/index/interrupt) function and the `Command` primitive.

### Basic human-in-the-loop workflow

We will create three [tasks](https://docs.langchain.com/oss/javascript/langgraph/functional-api#task):
...
We can now compose these tasks in an [entrypoint](https://docs.langchain.com/oss/javascript/langgraph/functional-api#entrypoint):
...
Short-term memory allows storing information across different **invocations** of the same **thread id**. See [short-term memory](https://docs.langchain.com/oss/javascript/langgraph/functional-api#short-term-memory) for more details.
...
An example of a simple chatbot using the functional API and the [`InMemorySaver`](https://reference.langchain.com/javascript/classes/_langchain_langgraph-checkpoint.MemorySaver.html) checkpointer. The bot is able to remember the previous conversation and continue from where it left off.
...
[long-term memory](https://docs.langchain.com/oss/javascript/concepts/memory#long-term-memory) allows storing information across different **thread ids**. This could be useful for learning information about a given user in one conversation and using it in another.

## Workflows

-   [Workflows and agent](https://docs.langchain.com/oss/javascript/langgraph/workflows-agents) guide for more examples of how to build workflows using the Functional API.
