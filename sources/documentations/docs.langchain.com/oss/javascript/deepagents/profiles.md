...
Replace the base Deep Agents system prompt (`CUSTOM` in [Prompt assembly](https://docs.langchain.com/oss/javascript/deepagents/customization#prompt-assembly)).

system\_prompt\_suffix

string

Append text to the assembled base prompt (`SUFFIX` in [Prompt assembly](https://docs.langchain.com/oss/javascript/deepagents/customization#prompt-assembly)); applied to the main agent, declarative subagents, and the auto-added general-purpose subagent.
...
When both a provider-level and a model-level profile exist, they are merged at resolution time. Unset model-level fields inherit from the provider-level profile; explicit model-level values override them. Re-registering under an existing key merges the new profile on top of the prior one—it does not replace it. See [Merge semantics](#merge-semantics) for the per-field rules.
...
-   [Harness](https://docs.langchain.com/oss/javascript/deepagents/harness) — overview of harness capabilities
-   [Models](https://docs.langchain.com/oss/javascript/deepagents/models) — configure model providers and parameters
-   [Customization](https://docs.langchain.com/oss/javascript/deepagents/customization) — full `create_deep_agent` configuration surface
