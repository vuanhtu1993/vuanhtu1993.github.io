---
title: "Permissions - Docs by LangChain"
source_url: "https://docs.langchain.com/oss/javascript/deepagents/permissions"
crawled_at: "2026-06-17T14:45:12.599Z"
---
...
Rules use first-match-wins evaluation: the first rule whose `operations` and `paths` match the current call determines the outcome. If no rule matches, the call is **allowed** (permissive default). Paths must be absolute (start with `/`) and cannot contain `..` or `~`. Invalid paths throw at agent construction time.
...
[Subagents](https://docs.langchain.com/oss/javascript/deepagents/subagents) inherit the parent agent’s permissions by default. To give a subagent different permissions, set the `permissions` field in its spec. This **replaces** the parent’s rules entirely.
