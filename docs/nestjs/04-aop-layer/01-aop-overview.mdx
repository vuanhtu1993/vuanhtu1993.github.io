...
graph TD
    ...
    Req(["Incoming HTTP Request"]) --> MW["1. Middleware<br/>Express / Fastify Layer"]:::mw
    MW --> Guard["2. Guard<br/>Authentication & Authorization"]:::gd
    Guard --> InterceptorBefore["3. Interceptor - Before<br/>Pre-controller Logic / Caching / Tracing"]:::in
    InterceptorBefore --> Pipe["4. Pipe<br/>Validation & Transformation"]:::pp
    Pipe --> Controller["5. Controller Handler<br/>Business Logic Processing"]:::hl
    Controller --> InterceptorAfter["6. Interceptor - After<br/>Response Mapping / Post-processing"]:::in
    
    subgraph ExceptionZone["Error Handling Zone"]
        Controller -.->|"Throw Error"| Filter["7. Exception Filter<br/>Error Formatting & Logging"]:::ef
        Guard -.->|"Reject"| Filter
        Pipe -.->|"Validation Failed"| Filter
    end

    InterceptorAfter --> Res(["HTTP Response"])
    Filter --> Res
```
