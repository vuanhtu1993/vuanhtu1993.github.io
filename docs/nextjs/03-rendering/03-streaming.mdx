...
1. **TTFB (Time to First Byte):** Giảm đột phá. Thay vì chờ DB query xong mới gửi byte đầu tiên, Server gửi Header/Layout tĩnh (Static Shell) ngay lập tức. TTFB trở nên cực thấp.
2. **FCP (First Contentful Paint):** Tương tự, trình duyệt vẽ ngay được Static Shell.
3. **LCP (Largest Contentful Paint):** CẦN CẨN THẬN! Nếu ảnh/banner lớn nhất (Hero Image) nằm bên trong một `<Suspense>` bị chậm, LCP sẽ rất tệ. **Rule of thumb:** Luôn để thành phần LCP nằm NGOÀI các Suspense boundary.
